...
- **Global Shared Capabilities**: Located in centralized directories (e.g. `src/configs/constants/`, `test/helpers/`) equipped with folder-wrapped barrel `index.ts`. Reserved exclusively for assets consumed across two or more independent feature domains.
...
- Never create nested subfolders or barrel wrappers for module-local concerns (e.g. do not create `base/constants/` or `stylistic/constants/`). Sibling files provide clean encapsulation without directory nesting debt.
- Primary files import sibling capabilities directly via `./<name>.<kind>.js`.
...
- Verify that module-local resources use `<name>.<kind>.ts` sibling naming without nested folders.
